function output = LineLength(data, windowLength, windowDisplacement, channel, fs, smoothDur, curTime)
...
% calculate number of sliding windows (overlap is ok)
  ...
  nw = int64(NumWins(length(data), fs, windowLength, windowDisplacement));
  timeOut = zeros(nw,1);
  featureOut = zeros(nw, channel);
...
%   filtOut = low_pass_filter(data, fs); % see below
  ...
  for w = 1: nw
    winBeg = windowDisplacement * fs * (w-1) + 1;
    winEnd = min([winBeg+windowLength*fs-1 length(filtOut)]);
    timeOut(w) = winEnd/fs*1e6 + curTime;         % right-aligned
    featureOut(w,:) = paramsFunction(filtOut(winBeg:winEnd,:)); 
  end
