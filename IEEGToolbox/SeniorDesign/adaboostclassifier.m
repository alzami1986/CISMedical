...
Y = logical(Y);
ens = fitensemble(X,Y,'AdaBoostM1',50,'Tree','Cost',[0 1; 5 0]);
mdl = crossval(ens);
misclass1 = kfoldLoss(mdl);
disp(['Ada CVal Accuracy: ' num2str(1.0 - misclass1)]);



% % create Adaboost ROC curve
% 
% [~,score_tree] = resubPredict(mdl);
% [XTree,YTree,TTree,AUCTree] = perfcurve(Y,score_tree(:,mdlTree.ClassNames),'true');
% 
% 
% % plot ROC curve
% figure
...
FP = C(1,2);
precision = TP / (TP + FP);
disp(['Ada Prediction Accuracy: ', num2str(accuracy)]);
disp(['Ada Precision: ', num2str(precision)]);
